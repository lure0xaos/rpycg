...
class CodeConverter(private val context: FxContext, private val settings: Settings) {
    ...
    fun toCode(menu: ModelItem): List<String> {
        ...
        context.toBuilder().setLocale(settings.getLocaleMenu()).build().use {
            messages =
                it.loadResources(CodeConverter::class) ?: throw CodeGenerationException("No resources {CodeConverter}")

        }
        val fileVariables =
            if (messages.containsKey(LC_FILE_VARIABLES)) messages.getString(LC_FILE_VARIABLES) else MSG_GAME_VARIABLES
        ...
        if (settings.getEnableWrite()) {
            val messageWritten =
                if (messages.containsKey(LC_MESSAGE_WRITTEN)) messages.getString(LC_MESSAGE_WRITTEN) else MSG_VARIABLES_WRITTEN
    ...
    private fun createCheatMenu(messages: ResourceBundle, root: ModelItem): List<String> {
        ...
        buffer += "        # nevermind"
        buffer += "        \"~#${if (messages.containsKey(LC_NEVERMIND)) messages.getString(LC_NEVERMIND) else MSG_NEVERMIND}~\":"
        buffer += "            return"
        return buffer
    }

    private fun createCheatSubmenu(
        indent: Int, messages: ResourceBundle, root: ModelItem, parentLabel: String
    ): List<String> {
        val buffer: MutableList<String> = mutableListOf()
            ...
            when {
                ModelType.VARIABLE === modelType -> {
                    ...
                    if (itemValue.isNotBlank()) {
                        ...
                        val prompt =
                            if (messages.containsKey(LC_MESSAGE_PROMPT)) messages.getString(LC_MESSAGE_PROMPT) else MSG_MESSAGE_PROMPT
                        buffer += indent(
                            indent,
                            "        $#${itemName} = #${itemTypeKeyword}(renpy.input(\"#${
                                format(prompt, mapOf("label" to itemLabel, "value" to "[$itemName]"))
                            }\").strip() or #${itemName})",
                        )
                    }
                ...
                ModelType.MENU === modelType -> {
                    ...
                    buffer += indent(
                        indent,
                        "                \"~#${if (messages.containsKey(LC_BACK)) messages.getString(LC_BACK) else MSG_BACK}~\":"
                    )
                    buffer += indent(indent, "                    jump #${parentLabel}")
                }
            }
