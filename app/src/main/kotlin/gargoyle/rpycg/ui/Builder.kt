...
class Builder : ScrollPane(), Initializable {
    ...
    init {
        component = FxContext.current.loadComponent(this) ?: throw AppUserException("No view {Builder}")
    }
    ...
    private fun addVariable(item: TreeItem<DisplayItem>) {
        ...
        stage.also { window: Stage? -> dialog.initOwner(window) }
        dialog.showAndWait().ifPresent { variable: DisplayItem -> addItem(item, variable, false) }
    }
        ...
        val treeCell: TreeCell<DisplayItem> =
            DisplayItemTreeCell { cell: TreeCell<DisplayItem>, displayItem: DisplayItem ->
                cell.text = displayItem.label.value.takeIf { it!!.isNotBlank() }
                    ?: displayItem.let { obj: DisplayItem -> obj.name.value }
    ...
    private fun createMenuItem(
        ...
        val item = MenuItem(resources.getString(key), ImageView(icon.findIcon(context)?.toExternalForm()))
    ...
    private fun createPlaceHolder(resources: ResourceBundle): Node {
        ...
        val placeholder = Button(
            resources.getString(LC_TEMPLATE),
            ImageView(Icon.TEMPLATE.findIcon(context)?.toExternalForm())
        )
        placeholder.tooltip = Tooltip(resources.getString(LC_TEMPLATE_TOOLTIP))
    ...
    private fun editVariable(item: TreeItem<DisplayItem>) {
        ...
        stage.also { window: Stage? -> dialog.initOwner(window) }
        dialog.setDisplayItem(item.value)
        dialog.showAndWait().ifPresent { variable: DisplayItem -> replaceItem(item, variable) }
    }

    private fun getKnownNames(allow: String): Set<String> {
        val known: MutableSet<String> = HashSet(tree.childrenUnmodifiable.size)
        visitItems(tree) { displayItem: DisplayItem ->
            val name = displayItem.name.value
    ...
    private fun onTemplate() {
        ...
        if (isTreeEmpty || context.confirm(
                stage, resources.getString(LC_TEMPLATE_CONFIRM),
                mapOf(
                    ButtonData.OK_DONE to resources.getString(LC_TEMPLATE_CONFIRM_OK),
                    ButtonData.CANCEL_CLOSE to resources.getString(LC_TEMPLATE_CONFIRM_CANCEL)
                )
            )
        ) {
            try {
                ...
                if (ButtonData.OTHER == context.error(
                        stage, resources.getString(LC_ERROR_MALFORMED_SCRIPT), e,
                        mapOf(
    ...
    private fun removeItem(treeItem: TreeItem<DisplayItem>) {
        if (FxContext.current.confirm(
                stage, resources.getString(LC_REMOVE_CONFIRM), mapOf(
                    ButtonData.OK_DONE to resources.getString(LC_REMOVE_CONFIRM_OK),
                    ButtonData.CANCEL_CLOSE to resources.getString(LC_REMOVE_CONFIRM_CANCEL)
                )
            )
        ) {
    ...
    private class DisplayItemTreeCell(
        ...
        override fun updateItem(item: DisplayItem?, empty: Boolean) {
            ...
            if (item != null) decorator(this, item)
        }
    }
    ...
    companion object {
        ...
        private fun getFullness(treeItem: TreeItem<DisplayItem>): FULLNESS {
            ...
            return when (item!!.modelType.value) {
