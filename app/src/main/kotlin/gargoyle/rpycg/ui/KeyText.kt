...
class KeyText : TextField() {
    ...
    init {
        val optional: ResourceBundle = FxContext.current.loadResources(KeyText::class)!!
        val tooltip = optional.getString(LC_DEFAULT) ?: LC_DEFAULT
        val pressKey = optional.getString(LC_PRESS_ANY_KEY) ?: LC_PRESS_ANY_KEY
        defaultCombination.addListener { _: ObservableValue<out KeyCodeCombination>, oldValue: KeyCodeCombination?, newValue: KeyCodeCombination ->
            if (null == oldValue) combination.value = newValue
    ...
    companion object {
        private val NONE = KeyCodeCombination(KeyCode.CODE_INPUT)
