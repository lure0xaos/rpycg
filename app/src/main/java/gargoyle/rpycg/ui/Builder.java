...
public final class Builder extends ScrollPane implements Initializable {
    ...
    public Builder() {
        ...
        component = FXContextFactory.currentContext().<Builder, ScrollPane>loadComponent(this)
                .orElseThrow(() -> new AppUserException("No view {resource}",
                        Map.of("resource", Builder.class.getName())));
    }
    ...
    private static FULLNESS getFullness(final TreeItem<DisplayItem> treeItem) {
        ...
        return switch (item.getModelType()) {
            case VARIABLE -> FULLNESS.determineFullness(null == treeItem.getParent() ? 0
                    : treeItem.getParent().getChildren().size());
            case MENU -> FULLNESS.determineFullness(treeItem.getChildren().size());
            default -> throw new IllegalArgumentException(String.valueOf(item.getModelType()));
        };
    }
    ...
    private static void setCellSignalDecorations(final Styleable cell, final TreeItem<DisplayItem> treeItem) {
        if (null != treeItem && null != treeItem.getValue()) {
            switch (getFullness(treeItem)) {
                case NORMAL -> Classes.classRemoveAll(cell, CLASS_WARN, CLASS_DANGER);
                case ALMOST -> Classes.classAddRemove(cell, CLASS_WARN, CLASS_DANGER);
                case FULL -> Classes.classAddRemove(cell, CLASS_DANGER, CLASS_WARN);
            }
        } else {
        ...
        if (null != treeItem) {
            ...
            if (null != displayItem) {
                switch (displayItem.getModelType()) {
                    case MENU -> Classes.classAddRemoveAll(cell, CLASS_MENU,
                    ...
                    default -> throw new IllegalStateException(String.valueOf(displayItem.getModelType()));
                }
            }
        }
    }
    ...
    private ContextMenu createContextMenuForCell(final TreeItem<DisplayItem> treeItem) {
        ...
        if (null != treeItem) {
            ...
            if (null != displayItem) {
                ...
                if (ModelType.VARIABLE == modelType) {
                    menuItems.add(createMenuItem(context, Icon.VARIABLE, "edit-variable", treeItem,
                            this::editVariable));
                }
                if (ModelType.MENU == modelType) {
                    menuItems.add(createMenuItem(context, Icon.MENU, "edit-menu", treeItem, this::editMenu));
                    menuItems.add(new SeparatorMenuItem());
                    menuItems.add(createMenuItem(context, Icon.VARIABLE, "create-variable", treeItem,
                            this::addVariable));
                    menuItems.add(createMenuItem(context, Icon.MENU, "create-menu", treeItem, this::addMenu));
                    menuItems.add(new SeparatorMenuItem());
                    menuItems.add(createMenuItem(context, Icon.MENU, "split-menu", treeItem, this::splitMenu));
                }
                if (null != treeItem.getParent()) {
                    menuItems.add(new SeparatorMenuItem());
                    menuItems.add(createMenuItem(context, Icon.DELETE, "remove", treeItem, this::removeItem));
                }
        ...
        menuItems.add(createMenuItem(context, Icon.VARIABLE, "create-variable", rootItem, this::addVariable));
        menuItems.add(createMenuItem(context, Icon.MENU, "create-menu", rootItem, this::addMenu));
        return new ContextMenu(menuItems.toArray(MENU_ITEMS));
    }
    ...
    private TreeCell<DisplayItem> createDisplayItemTreeCell(final TreeView<DisplayItem> treeView) {
        final TreeCell<DisplayItem> treeCell = new DisplayItemTreeCell((cell, displayItem) -> {
            ...
            Optional.ofNullable(displayItem)
                    .map(DisplayItem::getModelType).map(type -> switch (type) {
                        case MENU -> Icon.MENU;
    ...
    private MenuItem createMenuItem(final FXContext context, final Icon icon, final String key,
                                    final TreeItem<DisplayItem> treeItem,
                                    final Consumer<? super TreeItem<DisplayItem>> handler) {
        final MenuItem item = new MenuItem(resources.getString(key), Optional.ofNullable(icon)
                .flatMap(location -> icon.findIcon(context).map(URL::toExternalForm).map(ImageView::new))
                .orElse(null));
        item.setOnAction(event -> handler.accept(treeItem));
        return item;
    }

    private Node createPlaceHolder(final ResourceBundle resources) {
        final FXContext context = FXContextFactory.currentContext();
        final Button placeholder = new Button(resources.getString(LC_TEMPLATE),
                Icon.TEMPLATE.findIcon(context).map(URL::toExternalForm).map(ImageView::new).orElse(null));
