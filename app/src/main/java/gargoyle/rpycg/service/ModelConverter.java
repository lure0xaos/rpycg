...
public final class ModelConverter {
    public ModelItem toModel(final TreeItem<DisplayItem> root) {
        ...
        for (final TreeItem<DisplayItem> treeItem : root.getChildren()) {
            ...
            switch (value.getModelType()) {
                case MENU -> parent.addChild(toModel(treeItem));
                case VARIABLE -> parent.addChild(ModelItem.createVariable(
    ...
    public TreeItem<DisplayItem> toTree(final ModelItem item) {
        ...
        switch (modelType) {
            ...
            default -> throw new IllegalStateException(FXUtil.format("unsupported model type {type}",
                    Map.of("type", modelType)));
        }
    }
}
